...
var lists = function() {
    ...
    let sports=["旅游","篮球","足球","乒乓球","游泳","象棋","画画",
        "爬山","网球","跳舞","健身","排球",]
    i = Random.natural(20, 30);
    for (let x = 0; x < i; x++) {
        let m = date.getTime() + Random.natural(1000 * 60, 1000 * 60 * 60 * 24 * 30); //一月内
        ...
        let listData = {
            ...
            details:Random.cword(5,10),
            sport:sports[y],
            humanmax:Random.natural(0, 50),
            humannow:Random.natural(0, 100),
            thumbnail_pic_s: Random.image('200x100',  '#FFF', 'png', 'activity photo') // Random.dataImage( size, text ) 生成一段随机的 Base64 图片编码
        };
